...
for v in content.split('\n')[:-1]:
    print(v)
    linelist.append(pipe(v.split(',')[:-1]))
mindist = 0
print(linelist)
if len(linelist) == 2:
    line1 = linelist[0]
    ...
    for a in line1:
        for b in line2:
            ...
            if interx is not None and intery is not None and (mindist == 0 or abs(interx) + abs(intery) < mindist):
                mindist = abs(interx) + abs(intery)
